...
class RedisConn(object):
    ...
    def __init__(self, name, host, port=6379, db=0,
                 max_execs=3, timeout=10, block_size=256, slave_number=2):
        """Constructor"""
        self.name = name
        self.host = host
        self.port = port
    ...
    def _create_server(self):
        ...
        for c in range(self.slave_number-self.db-1):
            data['redis_slaves']['slave'+str(c)] = {
                'host': self.host, 'port': self.port, 'db': self.db+c}
        return redis.server.Server(data)
