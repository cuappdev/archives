...
def create_event_type(app_id, name, creator, fields_info):
  ...
  event_type = EventType(name=name, creator=creator,
                         application_id=app_id, fields_info=fields_info)
  db_utils.commit_model(event_type)
  return True, event_type

def get_event_type_by_name(app_id, name):
  return EventType.query.filter(EventType.application_id == app_id, EventType.name == name).first()

def get_event_type_by_id(event_type_id):
  return EventType.query.filter(EventType.id == event_type_id).first()

def verify_fields_info(fields_info):
  for field in fields_info:
    ...
    if 'type' not in metadata:
    ...
    elif type(metadata['required']) is not bool:
      ...
      elif type(metadata['default']).__name__ != metadata['type']:
        raise Exception('Default value invalid for field %s' % field)

  return True

def verify_fields(event_type_id, payload):
