...
class Path:
  ...
  def all_trips_from_stop_time(self, start, depart_time):
    trips = []
    trip = self.trip_from_stop_time(start, depart_time)
    while trip != []:
      trips.append(trip)
      trip = self.trip_from_stop_time(start, trip[-1].time)
    return trips
...
class Route:
  ...
  def __init__(self, route):
    ...
    paths = list(map(lambda x: Path(x), route.tables))
    # Map paths to days
